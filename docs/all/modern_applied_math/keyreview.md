...
- 非负可测函数的积分：
  ...
  $$
  \int_Ef(x)\text{d}x=\text{sup}\left\{ \int_E h(x)\text{d}x\vline h(x)\text{是非负可测简单函数，且}h(x)\leq f(x)\right\}
  $$
...
- **控制收敛定理**：
  ...
  > 最后，由不等式
  > $$
  > \vline \int_Ef(x)\text{d}x-\int_Ef_n(x)\text{d}x\vline\leq\int_Eg_n(x)\text{d}x
  > $$
